...
def mean_field_CRF(w_1, w_2, w_3, w_4, w_5, w_6, w_7, w_8, w_9, weight, num_iterations=10):
    ...
    boxes = tf.sigmoid(layers.Add()([Variable2(w_7, name="w_7")(boxes_in), layers.Multiply()([Variable2(w_8, name="w_8")(boxes_in), boxes_in])]))
    boxes_1 = tf.tile(tf.expand_dims(boxes, axis=-2), [1, 1, num_labels, 1])
    boxes_2 = tf.tile(tf.expand_dims(boxes, axis=-3), [1, num_labels, 1, 1])
    box_similarity = (tf.reduce_sum(tf.multiply(boxes_1, boxes_2), axis=-1) - 0.5) * 2
    ...
    pairwise_potentials = layers.Add()([#layers.Multiply()([-Variable2(w_1, name="w_1")(convexity_in), convexity_in]),
                                        ...
                                        layers.Multiply()([-Variable2(w_5, name="w_5")(box_similarity), box_similarity]),
